...
async function saveAction(formData: FormData) {
  ...
  const existing = await getTemplate(env, key);
  const isDynamic = (existing?.variables?.length ?? 0) > 0;
  if (isDynamic) {
    await upsertTemplate(env, key, { deepLink, enabled });
    await logAuditEvent(env, "notification.template_save", key, {
      enabled,
      deepLink,
      dynamic: true,
    });
...
export default async function TemplateEditPage({
  ...
  const template = await getTemplate(env, key);
