...
async function broadcastAction(formData: FormData) {
  ...
  if (!title || !body) {
    redirect("/notifications/broadcast?error=title_and_body_required");
  }
  if (tiers.length === 0) {
    redirect("/notifications/broadcast?error=select_at_least_one_tier");
  }
  ...
  try {
    ...
    await logAuditEvent(env, "notification.broadcast", null, {
      audience,
      title,
