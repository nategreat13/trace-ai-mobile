...
async function saveSpend(formData: FormData) {
  ...
  if (!platform || !month || !Number.isFinite(dollars)) {
    redirect("/spend?error=1");
  }

  const env = await getAdminEnv();
  const id = `${platform}_${month}`;
  const spendCents = Math.round(dollars * 100);
  await colRef(env, "adSpend").doc(id).set({
    platform,
    month,
    spendCents,
    updatedAt: new Date(),
  });
  await logAuditEvent(env, "spend.set", id, { platform, month, spendCents });

  revalidatePath("/analytics");
  revalidatePath("/spend");
  redirect("/spend?saved=1");
}
...
async function deleteSpend(formData: FormData) {
  ...
  if (!id) return;
  const env = await getAdminEnv();
  await colRef(env, "adSpend").doc(id).delete();
  await logAuditEvent(env, "spend.delete", id);
  revalidatePath("/analytics");
  revalidatePath("/spend");
}
