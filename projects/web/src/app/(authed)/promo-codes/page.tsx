...
async function generateAction(formData: FormData) {
  ...
  try {
    const created = await createPromoCode(env, {
      code,
      tier,
      durationDays,
      maxRedemptions,
    ...
    await logAuditEvent(env, "promo.create", created, {
      tier,
      durationDays,
      maxRedemptions,
...
async function disableAction(formData: FormData) {
  ...
  if (!code) return;
  await setPromoCodeActive(env, code, false);
  await logAuditEvent(env, "promo.disable", code);
  revalidatePath("/promo-codes");
}
...
async function enableAction(formData: FormData) {
  ...
  const code = formData.get("code") as string;
  if (!code) return;
  await setPromoCodeActive(env, code, true);
  await logAuditEvent(env, "promo.enable", code);
  revalidatePath("/promo-codes");
}
...
async function deleteAction(formData: FormData) {
  ...
  const code = formData.get("code") as string;
  if (!code) return;
  await deletePromoCode(env, code);
  await logAuditEvent(env, "promo.delete", code);
  revalidatePath("/promo-codes");
}
